...
static inline void _ocf_read_pt_submit(struct ocf_request *req)
{
	...
	OCF_DEBUG_RQ(req, "Submit");
	OCF_DEBUG_PARAM(req->cache, "Submitting PT request at pos: %llu, len: %u",
...
int ocf_read_pt_do(struct ocf_request *req)
{
	...
	OCF_DEBUG_PARAM(req->cache, "Starting PT read request at pos: %llu, len: %u",
		req->byte_position, req->byte_length);

	if (req->info.dirty_any) {
		OCF_DEBUG_PARAM(req->cache, "Request contains dirty data - cleaning first");
		ocf_hb_req_prot_lock_rd(req);
		/* Need to clean, start it */
		ocf_engine_clean(req);
		ocf_hb_req_prot_unlock_rd(req);
	...
	if (ocf_engine_needs_repart(req)) {
		OCF_DEBUG_RQ(req, "Re-Part");
		OCF_DEBUG_PARAM(req->cache, "Repartitioning required");
	...
	OCF_DEBUG_PARAM(req->cache, "PT read request processing complete");
...
int ocf_read_pt(struct ocf_request *req)
{
	...
	OCF_DEBUG_TRACE(req->cache);
	OCF_DEBUG_PARAM(req->cache, "Starting PT read at pos: %llu, len: %u",
		req->byte_position, req->byte_length);
	...
	if (req->seq_cutoff && ocf_engine_is_dirty_all(req) &&
			!req->force_pt) {
		use_cache = true;
		OCF_DEBUG_PARAM(req->cache, "Using cache due to sequential cutoff");
	} else {
		if (ocf_engine_mapped_count(req)) {
			OCF_DEBUG_PARAM(req->cache, "Mapped cache lines found: %u",
				ocf_engine_mapped_count(req));
			/* There are mapped cache line,
			 * lock request for READ access
			 */
			lock = ocf_req_async_lock_rd(
			...
			OCF_DEBUG_PARAM(req->cache, "No mapped cache lines");
			/* No mapped cache lines, no need to get lock */
			lock = OCF_LOCK_ACQUIRED;
		}
	}
	...
	if (use_cache) {
		OCF_DEBUG_PARAM(req->cache, "Switching to cache mode");
		/*
		 * There is dirt HIT, and sequential cut off,
		 * because of this force read data from cache
		 */
		...
		if (lock >= 0) {
			if (lock == OCF_LOCK_ACQUIRED) {
				// OCF_DEBUG_PARAM(req->cache, "Lock acquired - proceeding with PT read");
				/* Lock acquired perform read off operations */
				ocf_read_pt_do(req);
			} else {
				/* WR lock was not acquired, need to wait for resume */
				OCF_DEBUG_RQ(req, "NO LOCK");
				// OCF_DEBUG_PARAM(req->cache, "Lock not acquired - waiting for resume");
			}
		} else {
			OCF_DEBUG_RQ(req, "LOCK ERROR %d", lock);
			// OCF_DEBUG_PARAM(req->cache, "Lock error occurred: %d", lock);
			req->complete(req, lock);
			ocf_req_put(req);
		}
	}
