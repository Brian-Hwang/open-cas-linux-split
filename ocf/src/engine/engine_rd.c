...
void ocf_read_generic_submit_hit(struct ocf_request *req)
{
    ...
    cache_ios = (ocf_engine_io_count(req) * 8) / 10;  // 80% to cache
    volume_ios = ocf_engine_io_count(req) - cache_ios; // 20% to volume

    OCF_DEBUG_PARAM(req->cache, "Total IO count: %u, Cache IOs: %u, Volume IOs: %u",
        ocf_engine_io_count(req), cache_ios, volume_ios);
    OCF_DEBUG_PARAM(req->cache, "Request byte length: %u", req->byte_length);
...
// {
// 	/* Calculate cache vs volume split */
// 	uint32_t cache_ios = (ocf_engine_io_count(req) * 8) / 10;  // 80% to cache
// 	uint32_t volume_ios = ocf_engine_io_count(req) - cache_ios; // 20% to volume
...
// 	/* Submit cache requests */
// 	if (cache_ios > 0) {
// 		OCF_DEBUG_PARAM(req->cache, "Submitting to cache: %u IOs", cache_ios);
// 		ocf_submit_cache_reqs(req->cache, req, OCF_READ, 0,
// 			(req->byte_length * cache_ios) / ocf_engine_io_count(req),
// 			cache_ios, _ocf_read_generic_hit_complete);
// 	}
	
// 	/* Submit volume requests */
// 	if (volume_ios > 0) {
// 		OCF_DEBUG_PARAM(req->cache, "Submitting to volume: %u IOs", volume_ios);
...
int ocf_read_generic(struct ocf_request *req)
{
	...
	OCF_DEBUG_PARAM(cache, "Starting read request at pos: %llu, len: %u",
		req->byte_position, req->byte_length);

	ocf_io_start(&req->ioi.io);

	if (env_atomic_read(&cache->pending_read_misses_list_blocked)) {
		OCF_DEBUG_PARAM(cache, "Cache bypassed - pending reads blocked");
	...
	// OCF_DEBUG_PARAM(cache, "Lock status: %d, Mapping error: %d",
		// lock, ocf_req_test_mapping_error(req));

	if (!ocf_req_test_mapping_error(req)) {
		if (lock >= 0) {
			if (lock != OCF_LOCK_ACQUIRED) {
				// OCF_DEBUG_PARAM(cache, "NO LOCK - waiting for resume");
				/* Lock was not acquired, need to wait for resume */
			} else {
				// OCF_DEBUG_PARAM(cache, "Lock acquired - proceeding with IO");
				/* Lock was acquired can perform IO */
				_ocf_read_generic_do(req);
			}
		} else {
			// OCF_DEBUG_PARAM(cache, "LOCK ERROR %d", lock);
			req->complete(req, lock);
			ocf_req_put(req);
		}
	} else {
		// OCF_DEBUG_PARAM(cache, "Mapping error - falling back to PT");
		ocf_req_clear(req);
		req->force_pt = true;
		ocf_get_io_if(ocf_cache_mode_pt)->read(req);
	}
